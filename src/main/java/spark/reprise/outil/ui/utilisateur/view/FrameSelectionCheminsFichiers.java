package spark.reprise.outil.ui.utilisateur.view;
...
import spark.reprise.outil.ui.utilisateur.controller.FrameSelectionCheminsFichiersController;
...
public class FrameSelectionCheminsFichiers extends javax.swing.JPanel {
    ...
    String lastPath = "";
    final List<TreeCellLineEditor> editeursFichier = new ArrayList<TreeCellLineEditor>();
    ...
    public FrameSelectionCheminsFichiers(
	    FrameSelectionCheminsFichiersController controller) {
	super();
	initComponents();
	...
	final TreeCellLineEditor fichier = new TreeCellLineEditor(listFichier1,
		nom,extension);
	...
	fichier.addEnabledListener(new ActionListener() {
		...
		controller.setEnabled(nom, editeursFichier.get(i)
			.getFichierEnabled());
	...
	fichier.addCheminListener(new DocumentListener() {
		...
		try {
			...
			final String text=arg0.getDocument().getText(0,
	...
	final javax.swing.JLabel l1 = new javax.swing.JLabel(resourceMap
		.getString("columnFile.header"));
	gridBagConstraints = new java.awt.GridBagConstraints();
	gridBagConstraints.gridx = 0;
	gridBagConstraints.gridy = 0;
	final Font headerFont = new Font(getFont().getName(), Font.BOLD, getFont()
		.getSize() + 1);

	l1.setFont(headerFont);
	listFichier1.add(l1, gridBagConstraints);

	final javax.swing.JLabel l2 = new javax.swing.JLabel(resourceMap
		.getString("columnPath.header"));
	...
	protected List<ActionListener> tcleListeEnabledListener = new ArrayList<ActionListener>();

	protected List<DocumentListener> tcleListeCheminListener = new ArrayList<DocumentListener>();
...
//	    this.pos = pos;
	...
	private void initComponents() {
	    ...
	    jtxtfldEmplacementFichier.getDocument().addDocumentListener(
	    ...
	    jtxtfldEmplacementFichier.setBorder(new LineBorder(
	...
	private void disableLine(boolean etat) {
	    ...
	    jChckFichierActif.setBackground(jtxtfldEmplacementFichier
		    .getBackground());
	}
	...
	void changerEmplacement() {
	    ...
	    jtxtfldEmplacementFichier
    	    .setText(controller.getChemin(nom));
	}
