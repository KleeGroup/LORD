...
public class DialogMultiColumnConstraintsModel {
    private static class CommandeConstraint{
	    ...
	    boolean isValide() {
		final boolean test= id!=null && !("".equals(id))
			&& method!=null && errorMessage!=null
			&& cols!=null && cols.size()>0;
		if("Unique".equals(method)){
		    return test;
		}
		return test && ContrainteMultiColFonctionsSpecifiques.
			isValide(method, cols.toArray(new String[cols.size()]));
	...
	List<CommandeConstraint> commandes=new ArrayList<CommandeConstraint>();
	

	private final String nomColonne[] = new String[] { "Id verif", "Fonction",
		"Colonnes", "Message d'erreur", };
	...
	private CommandeConstraint transform(ContrainteMultiCol cm) {
	    ...
	    cc.id=cm.getID();
	    cc.cols=new ArrayList<String>(cm.getNomColonnes());
	...
	private List<String> trans(List<Colonne> value) {
	    final List<String> liste=new ArrayList<String>(value.size());
	...
	final List<String> pmn= new ArrayList<String>(ContrainteMultiColFonctionsSpecifiques.getMethods());
	pmn.add("Unique");
