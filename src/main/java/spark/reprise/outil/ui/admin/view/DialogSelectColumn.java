...
public class DialogSelectColumn extends javax.swing.JDialog {
	...
	ActnClear() {
	    super("", new ImageIcon(ClassLoader.
			getSystemResource("resources/images/clear.png")));
	}
    ...
    public DialogSelectColumn(DialogSelectColumnController controller,
	...
	if (singleSelectMode) {
	    jtreeCols.getSelectionModel().setSelectionMode(
	...
	if (!singleSelectMode
		&& (e.getClickCount() == 2 || (e.getModifiers() & MouseEvent.CTRL_MASK) != 0)
		|| singleSelectMode) {
	    final TreePath tp = jtreeCols.getPathForLocation(e.getX(), e
		    .getY());
