...
public class DialogSelectColumnSchemaModel implements IColumnSeclect {
    ...
    private final List<Colonne> chosenElements=new ArrayList<Colonne>();
    private final SchemaTreeModel stm;
    
    private static class SchemaTreeModel implements javax.swing.tree.TreeModel {

	    private final List<TreeModelListener> tml = new ArrayList<TreeModelListener>();

	    private List<Fichier> fichiers = new ArrayList<Fichier>();
	    private final Map<String,List<Colonne>> listeColonnes=new HashMap<String,List<Colonne>>();
		...
		fichiers=new ArrayList<Fichier>(s.getFichiers());
//		fichiers.remove(f);
		for(final Fichier fic:fichiers){
		    List<Colonne> listColonne=null;
		    listColonne= new ArrayList<Colonne>(fic.getColonnes());
		    if(fic==f){
...
/**{@inheritDoc}*/
		...
		for(int j=0;j<fichiers.size();j++){
			...
			if (!(listeCol.get(i).getNom().toLowerCase().contains(filter)
			||listeCol.get(i).getDescription().toLowerCase().contains(filter))){
			    listeCol.remove(i);
			    i--;
			}
		    }
...
/**{@inheritDoc}*/
	...
	if (chosenElements.size()>0){
	return "["+chosenElements.get(0).getFichierParent()+"].["
		+chosenElements.get(0).getNom()+"]";
	}
	return "";
    }
/**{@inheritDoc}*/
	...
	return new ArrayList<Colonne>(chosenElements);
    }
/**{@inheritDoc}*/
    @Override
